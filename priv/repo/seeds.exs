...
if count_admin == 0 do
  ...
  # create family owner
  {:ok, owner} = {:ok, %{admin_user: admin_user, user: _user, session: _session}} =
    AuthManager.create_normal_user("ngnclht@gmail.com", "Ancestortree@2020")

  # create family
  {:ok, family} = FamilyManager.create_new_family(admin_user, %{
    "name" => "Nguyễn Sỹ - Can Lộc Hà Tĩnh",
    "main_address" => "Can Lộc Hà Tĩnh",
    "description" => ""
  })
  {:ok, coLai} = family |> PersonManager.create_new_person(owner.user, %{
  ...
  {:ok, ongMai} = family |> PersonManager.create_new_person(owner.user, %{
  ...
  {:ok, baMai} = family |> PersonManager.create_new_person(owner.user,%{
  ...
  {:ok, boTruc} = family |> PersonManager.create_new_person(owner.user, %{
  ...
  {:ok, meHai} = family |> PersonManager.create_new_person(owner.user, %{
  ...
  {:ok, chuHoang} = family |> PersonManager.create_new_person(owner.user, %{
  ...
  {:ok, muHang} = family |> PersonManager.create_new_person(owner.user, %{
